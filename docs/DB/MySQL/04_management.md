...
_______________________________________________________________________________________________________
# 権限付与（ローカル環境以外では実行注意）
ユーザ "myUser01" に、全ての権限を付与（全データベースの全権限＋他ユーザへ権限付与する権限まで丸ごと付加）
```
GRANT ALL PRIVILEGES ON *.* TO 'myUser01'@'%' WITH GRANT OPTION;
FLUSH PRIVILEGES;
```
...
_______________________________________________________________________________________________________
# 権限確認
ユーザ "myUser01" が保持している権限一覧
```
SHOW GRANTS FOR 'myUser01'@'%' \G
```
