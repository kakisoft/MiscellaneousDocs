...
（xは、パスワードが必要という意味）
x・・・パスワードのプレースホルダー（実際のパスワードは /etc/shadow に保存される）
/sbin/nologin: ・・・ この場合はログインを許可しないシェル
...
```
・内部コマンド
シェル事態に組み込まれているコマンド。シェルが直接処理するため、外部プログラムを呼び出す必要が無い。

・外部コマンド
シェルとは別に存在する外部プログラム。

type コマンドで、内部コマンドなのか外部コマンドなのかを判別可。
```
